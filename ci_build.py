...
# conanbuild.bat
# cmake ..  -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE="conan_toolchain.cmake" -DCMAKE_INSTALL_PREFIX="D:\3d-source-build\drogon\drogon1.9.11\drogon\build\install"
# conanrun.bat
...
def main():
    parser = argparse.ArgumentParser(description='Drogon跨平台构建脚本')
    ...
    try:
        ...
        run_command(conan_install_cmd, shell_type=False)
        
        # 3. 执行Conan环境设置
        if platform.system() == "Windows":
            run_command("conanbuild.bat")
        else:
            run_command("source conanbuild.sh")
        ...
        # 5. 执行Conan运行环境设置
        if platform.system() == "Windows":
            run_command("conanrun.bat")
        else:
            run_command("source conanrun.sh")
